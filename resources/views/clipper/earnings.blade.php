...
<x-app-layout>
    <x-slot name="header">
        <h1 class="font-display text-2xl font-bold text-ink-900">Mes revenus</h1>
        <p class="mt-1 text-ink-500">Versements sur {{ $clipper->paypal_email }}</p>
    </x-slot>
    ...
    <div class="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            ...
            <div class="card p-6">
                <h2 class="font-display text-lg font-bold text-ink-900">Demander un retrait</h2>
                <div class="mt-4">
            ...
            <div class="card lg:col-span-2">
                <div class="border-b border-ink-100 px-6 py-4">
                    <h2 class="font-display text-lg font-bold text-ink-900">Mes retraits</h2>
                </div>

                @if ($payouts->isEmpty())
                    <p class="px-6 py-10 text-center text-sm text-ink-500">Aucun retrait demandé.</p>
                @else
                    <ul class="divide-y divide-ink-100">
                        @foreach ($payouts as $payout)
                            <li class="flex flex-wrap items-center justify-between gap-3 px-6 py-4">
                                <div>
                                    <p class="font-display font-bold tabular text-ink-900">
                                    ...
                                    @if ($payout->failure_reason)
                                        <p class="mt-1 text-sm text-red-600">{{ $payout->failure_reason }}</p>
                                    @endif
                                </div>
        ...
        <div class="card">
            <div class="border-b border-ink-100 px-6 py-4">
                <h2 class="font-display text-lg font-bold text-ink-900">Historique des crédits</h2>
                <p class="mt-0.5 text-sm text-ink-400">Chaque ligne correspond à des vues rémunérées.</p>
            </div>

            @if ($transactions->isEmpty())
                <p class="px-6 py-10 text-center text-sm text-ink-500">Aucun crédit pour le moment.</p>
            @else
                        ...
                        <tbody class="divide-y divide-ink-100">
                            @foreach ($transactions as $transaction)
                                <tr>
                                    ...
                                    <td class="px-6 py-3 text-ink-800">{{ $transaction->campaign?->title }}</td>
                                    <td class="px-6 py-3 text-right tabular text-ink-500">{{ Money::views($transaction->views_delta) }}</td>
                                    <td @class([
                                        'px-6 py-3 text-right font-semibold tabular',
                                        'text-red-600' => $transaction->amount_cents < 0,
                                        'text-ink-900' => $transaction->amount_cents >= 0,
