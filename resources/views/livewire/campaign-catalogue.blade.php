...
<div class="space-y-6">
    ...
    {{--
        Squelettes pendant un filtrage.

        L'opacité seule laissait lire les anciens résultats en transparence :
        on croyait la liste à jour alors qu'elle montrait encore la
        précédente. Dessiner la forme de ce qui arrive dit clairement que le
        contenu affiché n'est plus le bon, et la page ne saute pas quand les
        vraies cartes se posent.
    --}}
    <div wire:loading.delay.long class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3" aria-hidden="true">
        @for ($i = 0; $i < 6; $i++)
    ...
    @else
        <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3" wire:loading.remove.delay.long>
